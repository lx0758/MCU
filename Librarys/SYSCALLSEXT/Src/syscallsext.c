...
UART_HandleTypeDef *gSysCallsExtHuart;

void SYSCALLSEXT_Init(UART_HandleTypeDef *huart)
{
  gSysCallsExtHuart = huart;
...
__attribute__((unused)) int _write(int fd, char *ptr, int len)
{
  ...
  {
    hstatus = HAL_UART_Transmit(gSysCallsExtHuart, (uint8_t *) ptr, len, HAL_MAX_DELAY);
    if (hstatus == HAL_OK)
      return len;
    else
      return EIO;
...
__attribute__((unused)) int _read(int fd, char *ptr, int len)
{
  ...
  if (fd == STDIN_FILENO)
  {
    hstatus = HAL_UART_Receive(gSysCallsExtHuart, (uint8_t *) ptr, len, HAL_MAX_DELAY);
    if (hstatus == HAL_OK)
      return len;
    else
      return EIO;
